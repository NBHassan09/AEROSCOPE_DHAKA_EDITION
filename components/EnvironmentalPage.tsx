        ...
        <div className="p-8 h-full overflow-y-auto bg-gray-900 text-gray-200">
            <div className="max-w-7xl mx-auto">
                <div className="flex items-center space-x-3 mb-6">
                    ...
                    <h1 className="text-3xl font-bold text-gray-100">Environmental Dashboard</h1>
                </div>
                ...
                {/* Chart */}
                <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700 shadow-xl h-[500px]">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
                    <div className="lg:col-span-2 bg-gray-800/50 p-6 rounded-lg border border-gray-700">
                                ...
                                <div key={name} className="bg-gray-900/40 p-4 rounded-lg border border-gray-700/50">
                                    ...
                                    <div className="space-y-3 text-sm">
                                        <div className="flex justify-between items-center">
                                            <span className="text-gray-400 flex items-center"><Thermometer size={14} className="mr-1.5" /> Latest NO₂:</span>
                                            <span className="font-mono text-teal-300">{summaryStats[name]?.latest ? formatScientificJSX(summaryStats[name].latest.no2) : 'N/A'}</span>
                                        </div>
                                         <div className="flex justify-between items-center">
                                            <span className="text-gray-400 flex items-center"><TrendingUp size={14} className="mr-1.5" /> Peak NO₂:</span>
                                            <span className="font-mono text-red-400">
                                                {summaryStats[name]?.peak ? formatScientificJSX(summaryStats[name].peak.no2) : 'N/A'}
                                                <span className="text-gray-500 ml-1">({summaryStats[name]?.peak?.date})</span>
                                            </span>
                                        </div>
                                    </div>
                                </div>
                     ...
                     <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700">
                        <h3 className="text-2xl font-semibold flex items-center mb-4 text-red-400">
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                           {alerts.map(({name, point}, index) => (
                                <div key={index} className="flex justify-between items-center bg-red-900/20 p-2 rounded-md text-sm">
                                    <div>
                                        <span className="font-semibold text-gray-300">{name}</span>
                                        <span className="text-gray-400 ml-2">({point.date})</span>
                                    </div>
                                    <span className="font-mono text-red-300">{formatScientificJSX(point.no2)}</span>
                                </div>
                           ))}
                        </div>
                    </div>
